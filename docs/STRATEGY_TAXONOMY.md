...
For structurally novel products, `TraderFactory` may also emit a blueprint-driven stub rather than a runnable production sleeve.
That means the repo knows the family well enough to scaffold the design work, but not well enough to pretend the final strategy should be autogenerated.
...
- runnable `derivative_mm` support for vanilla options when explicit underlying, strike, expiry, and volatility inputs are declared
- blueprint-driven `derivative_stub` scaffolding for future rounds that need deliberate pricing logic before optimization
...
- Relativistic Black-Scholes and similar advanced PDE variants should remain `reference_only` until a real options round shows that plain Black-Scholes plus simple surface adjustments is not enough.
...
TraderFactory status:
- `factory_ready` for simple linked spreads and explicit weighted baskets

Blueprint support:
- simple two-leg linked products can now scaffold into runnable `spread_mm`
- explicit weighted baskets can now scaffold into runnable `basket_mm`
- broader linked structures can still scaffold into `spread_stub` when deliberate multi-leg design is required
...
Blueprint support:
- participant-driven products with explicit tracked participants can now scaffold into runnable `participant_mm`
- participant-driven products with unresolved identity semantics still scaffold into `participant_stub`
...
Blueprint support:
- `conversion_stub`, `auction_stub`, `storage_stub`, and `signal_stub` now exist as deliberate design scaffolds for future rounds with these mechanics
- simple conversion-observation products can now scaffold into runnable `conversion_mm`
- explicit external-signal products can now scaffold into runnable `signal_mm`
- explicit weighted baskets can now be declared via `basket_definition`
- participant-driven products can now declare tracked participants via `participant_rule`
- richer convertible or transport-heavy products should remain in `conversion_stub` until their mechanics are explicit
